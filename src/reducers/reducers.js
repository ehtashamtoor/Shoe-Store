...
export const ShoppingCartReducer = createSlice({
    ...
    reducers: {
        ADD_ITEM(state, { payload }) {
            ...
            const selected_item = state.products[payload - 1];
            let item = state.added_items.find(item => payload === item.id);

            if (item) {
                console.log('item ha')
                item.quantity += 1;
                state.total += selected_item.price;
            } else {
                console.log('item ni ha')
                state.added_items.push(selected_item)
                item = selected_item;
                item.quantity = 1;
                state.total += selected_item.price;
            }
        ...
        INC_ITEM(state, { payload }) {
            const selected_item = state.added_items[payload];
        ...
        DEC_ITEM(state, { payload }) {
            ...
            if (selected_item.quantity > 1) {
                selected_item.quantity -= 1;
                state.total -= selected_item.price;
            } else if (selected_item.quantity === 1) {
                selected_item.quantity -= 1;
                state.total -= selected_item.price;
                state.added_items.splice(payload, 1);
            }
        },
        DISMANTLE_ITEM(state, { payload }) {
            const selected_item = state.added_items[payload];
            state.added_items.splice(payload, 1)
            state.total -= (selected_item.price * selected_item.quantity);
        },
        RESET_STATE(state ){
            state.added_items = []
            state.total = 0
...
export const { ADD_ITEM, INC_ITEM, DEC_ITEM, DISMANTLE_ITEM, RESET_STATE } = ShoppingCartReducer.actions;
